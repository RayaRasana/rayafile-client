...
void DaemonManager::startSeafileDaemon()
{
    ...
    const QString config_dir = rayafileApp->configurator()->ccnetDir();
    const QString seafile_dir = rayafileApp->configurator()->seafileDir();
    const QString worktree_dir = rayafileApp->configurator()->worktreeDir();
...
void DaemonManager::checkDaemonReady()
{
    ...
    if (++maxcheck > kMaxDaemonReadyCheck) {
        ...
        rayafileApp->errorAndExit(tr("%1 client failed to initialize").arg(getBrand()));
    }
}
...
void DaemonManager::scheduleRestartDaemon()
{
    ...
    int max_retry = 2;
    if (rayafileApp->rpcClient() && rayafileApp->rpcClient()->isConnected()) {
    ...
    if (++restart_retried_ >= max_retry) {
        ...
        rayafileApp->errorAndExit(tr("%1 exited unexpectedly").arg(getBrand()));
        return;
    }
