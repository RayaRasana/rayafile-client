...
void LastSyncError::start()
{
    ...
    QString db_path = QDir(rayafileApp->configurator()->seafileDir()).filePath("sync_error.db");
    if (sqlite3_open (toCStr(db_path), &db)) {
        ...
        rayafileApp->errorAndExit(QObject::tr("failed to open sync error id database"));
        return;
    }
...
{
    QString account_sig = rayafileApp->accountManager()->currentAccount().getSignature();
