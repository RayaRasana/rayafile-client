...
{
    return expandVars(expandUser(rayafileApp->readPreconfigureEntry(kPreconfigureDirectory).toString()));
}
...
void Configurator::initConfig()
{
    ...
    if (!ccnet_dir.mkpath(".")) {
        rayafileApp->errorAndExit(tr("Error when creating ccnet configuration"));
        return;
    }
...
int Configurator::readSeafileIni(QString *content)
{
    ...
    if (!seafile_ini.open(QIODevice::ReadOnly | QIODevice::Text)) {
        rayafileApp->errorAndExit(tr("failed to read %1").arg(seafile_ini.fileName()));
    }
