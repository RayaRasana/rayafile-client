...
#else
    rayafileApp->configurator()->checkInit();
    return QDir(rayafileApp->configurator()->seafileDir()).filePath(kSeafileAppletSockName);
#endif
}
...
void SeafileAppletRpcServer::start()
{
    ...
    if (searpc_named_pipe_server_start(priv_->pipe_server) < 0) {
        ...
        rayafileApp->errorAndExit("failed to initialize");
    } else {
...
{
    rayafileApp->mainWindow()->showWindow();
}
