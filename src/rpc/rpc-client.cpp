...
#else
    return QDir(rayafileApp->configurator()->seafileDir()).filePath(kSeafileSockName);
#endif
}
...
bool SeafileRpcClient::connectDaemon(bool exit_on_error)
{
    ...
    while (true) {
        ...
        if (!seafile_rpc_client_) {
            if (retry++ > 20) {
                if (exit_on_error) {
                    rayafileApp->errorAndExit(tr("internal error: failed to connect to daemon"));
                }
                return false;
            } else {
                g_usleep(500000);
...
int SeafileRpcClient::listLocalRepos(std::vector<LocalRepo> *result)
{
    ...
    for (size_t i = 0; i < (*result).size(); i++) {
        ...
        repo.account = rayafileApp->accountManager()->getAccountByRepo(repo.id, this);
    }
