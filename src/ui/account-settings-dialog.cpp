...
void AccountSettingsDialog::onSubmitBtnClicked()
{
    ...
    if (url != account_.serverUrl.toString()) {
        ...
        if (rayafileApp->accountManager()->replaceAccount(account_,
            new_account) < 0) {
        ...
        if (rayafileApp->rpcClient()->updateReposServerHost(url,
    ...
    rayafileApp->messageBox(tr("Successfully updated current account information"), this);
    accept();
}
