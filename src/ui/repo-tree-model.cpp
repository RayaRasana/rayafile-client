...
void RepoTreeModel::setRepos(const std::vector<ServerRepo>& repos)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (repo.isSubfolder() || rayafileApp->rpcClient()->hasLocalRepo(repo.id))
...
void RepoTreeModel::refreshRepoItem(RepoItem *item, void *data)
{
    ...
    rayafileApp->rpcClient()->getLocalRepo(item->repo().id, &local_repo);
                                    ...
                                    const QModelIndex &right) const
{
    ...
    if (item_l->type() == REPO_CATEGORY_TYPE) {
        ...
        int order = rayafileApp->settingsManager()->repoSortOrder();
