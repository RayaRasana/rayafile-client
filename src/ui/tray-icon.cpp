...
public:
    ...
    void run()
    {
        ...
        rayafileApp->rpcClient()->getCommitDiff(
...
void SeafileTrayIcon::prepareContextMenu()
{
    ...
    bool success = rayafileApp->rpcClient()->getSyncErrors(&errors, 0, 1);
    if (!success) {
...
{
    if (rotate_counter_ >= 8 || !rayafileApp->settingsManager()->autoSync()) {
        rotate_timer_->stop();
        if (!rayafileApp->settingsManager()->autoSync())
...
{
    MainWindow *main_win = rayafileApp->mainWindow();
    main_win->showWindow();
}
...
//     QMessageBox::about(rayafileApp->mainWindow(), tr("About %1").arg(getBrand()),
...
{
    openUrl(QUrl::fromLocalFile(QFileInfo(rayafileApp->configurator()->seafileDir()).path()));
}
...
void SeafileTrayIcon::shellExtFix()
{
    ...
    QString log_dir = QDir(rayafileApp->configurator()->ccnetDir()).absoluteFilePath("logs");
    ...
    if (res == 0) {
        rayafileApp->warningBox(tr("Successfully fixed sync status icons for Explorer"));

    } else {
        rayafileApp->warningBox(tr("Failed to fix sync status icons for Explorer"));
...
{
    QString log_path = QDir(rayafileApp->configurator()->ccnetDir()).absoluteFilePath("logs");
...
void SeafileTrayIcon::uploadLogDirectory()
{
    if (!rayafileApp->accountManager()->currentAccount().isValid()) {
        rayafileApp->warningBox(tr("Please login first"));
        return;
    }
...
{
    rayafileApp->settingsDialog()->show();
    rayafileApp->settingsDialog()->raise();
    rayafileApp->settingsDialog()->activateWindow();
}
...
{
    rayafileApp->settingsManager()->setAutoSync(false);
}

void SeafileTrayIcon::enableAutoSync()
{
    rayafileApp->settingsManager()->setAutoSync(true);
}
...
void SeafileTrayIcon::refreshTrayIconToolTip()
{
    ...
    if (rayafileApp->rpcClient()->getUploadRate(&up_rate) < 0 ||
        rayafileApp->rpcClient()->getDownloadRate(&down_rate) < 0) {
        return;
    }
...
void SeafileTrayIcon::onMessageClicked()
{
    ...
    if (repo_id_.isEmpty())
        return;
    LocalRepo repo;
    if (rayafileApp->rpcClient()->getLocalRepo(repo_id_, &repo) != 0 ||
