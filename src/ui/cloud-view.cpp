...
void CloudView::createTabs()
{
    ...
    // rayafileApp->accountManager()->accounts().front().isPro();
    // if (has_pro_account) {
    //     addActivitiesTab();
    // }
}
...
void CloudView::showCreateRepoDialog(const QString& path)
{
    const std::vector<Account>& accounts =
        rayafileApp->accountManager()->accounts();
    if (accounts.empty()) {
        return;
    }
...
{
    rayafileApp->mainWindow()->showMinimized();
}

void CloudView::onCloseBtnClicked()
{
    rayafileApp->mainWindow()->hide();
}
...
bool CloudView::eventFilter(QObject* obj, QEvent* event)
{
    if (obj == mHeader) {
        if (event->type() == QEvent::MouseButtonPress) {
            QWindow* win = rayafileApp->mainWindow()->windowHandle();
            win->startSystemMove();
        }
    }
...
{
    return rayafileApp->accountManager()->hasAccount();
}
...
{
    int count = 0;
    if (rayafileApp->rpcClient()->getCloneTasksCount(&count) < 0) {
        return;
    }
...
void CloudView::refreshTransferRate()
{
    ...
    if (rayafileApp->rpcClient()->getDownloadRate(&down_rate) < 0) {
        return;
    }
...
{
    if (!rayafileApp->mainWindow()->isVisible()) {
        return;
    }
    refreshTasksInfo();
    refreshTransferRate();
...
void CloudView::onAccountChanged()
{
    ...
    if (account.isValid() && account.isPro()) {
        if (!account.serverInfo.customBrand.isEmpty()) {
            ...
            if (rayafileApp->mainWindow()) {
                rayafileApp->mainWindow()->setWindowTitle(title);
            }
        }
    ...
    repos_tab_->refresh();
    starred_files_tab_->refresh();
    if (rayafileApp->accountManager()->currentAccount().isPro()) {
        activities_tab_->refresh();
    }
    search_tab_->reset();
...
{
    if (account == rayafileApp->accountManager()->currentAccount()) {
        updateStorageUsage(account);
        account_view_->onAccountChanged();
    }
}
