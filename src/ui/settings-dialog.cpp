...
{
    SettingsManager *mgr = rayafileApp->settingsManager();
    ...
    if (language_changed && rayafileApp->yesOrNoBox(tr("You have changed languange. Restart to apply it?"), this, true))
        rayafileApp->restartApp();

    // if (proxy_changed && rayafileApp->yesOrNoBox(tr("You have changed proxy settings. Restart to apply it?"), this, true))
    //     rayafileApp->restartApp();
...
void SettingsDialog::autoStartChanged(int state)
{
    ...
    rayafileApp->settingsManager()->setAutoStart(autoStart);
}
...
void SettingsDialog::hideDockIconChanged(int state)
{
    ...
    rayafileApp->settingsManager()->setHideDockIcon(hideDockIcon);
}
...
void SettingsDialog::notifyChanged(int state)
{
    ...
    rayafileApp->settingsManager()->setNotify(notify);
}

void SettingsDialog::downloadChanged(int value)
{
    qDebug("%s :%d", __func__, value);
    rayafileApp->settingsManager()->setMaxDownloadRatio(mDownloadSpinBox->value());
}

void SettingsDialog::uploadChanged(int value)
{
    qDebug("%s :%d", __func__, value);
    rayafileApp->settingsManager()->setMaxUploadRatio(mUploadSpinBox->value());
}
...
bool SettingsDialog::validateProxyInputs()
{
    ...
    if (proxy_host.isEmpty()) {
        rayafileApp->warningBox(tr("The proxy host address can't be empty"),
                               this);
        return false;
    }
    ...
    int proxy_port = mProxyPort->value();
    if (proxy_port == 0) {
        rayafileApp->warningBox(tr("The proxy port is incorrect"),
                               this);
        return false;
    }
    ...
    if (mProxyRequirePassword->checkState() == Qt::Checked) {
        ...
        if (proxy_username.isEmpty()) {
            rayafileApp->warningBox(tr("Proxy username can't be empty"), this);
            return false;
        } else if (proxy_password.isEmpty()) {
            rayafileApp->warningBox(tr("Proxy password can't be empty"), this);
            return false;
        }
    }
