...
void CreateRepoDialog::createAction()
{
    ...
    if (!passwd_.isEmpty() && account_.isAtLeastVersion(4, 4, 0)) {
        ...
        pwd_hash_algo = rayafileApp->accountManager()->currentAccount().getEncryptedLibraryPwdHashAlgo();
        pwd_hash_params = rayafileApp->accountManager()->currentAccount().getEncryptedLibraryPwdHashAlgo();

        int enc_version = rayafileApp->accountManager()->currentAccount().getEncryptedLibraryVersion();

        if (enc_version < 2) {
            rayafileApp->warningBox(tr("Creating a library with encryption version less than 2 is not supported"), this);
            return;
        }

        if (rayafileApp->rpcClient()->generateMagicAndRandomKey(
                enc_version, repo_id, passwd_, pwd_hash_algo, pwd_hash_params, &magic, &pwd_hash, &random_key, &salt) < 0) {
            rayafileApp->warningBox(tr("Failed to generate encryption key for this library"), this);
            return;
        }
...
bool CreateRepoDialog::validateInputs()
{
    ...
    if (path.isEmpty()) {
        rayafileApp->warningBox(tr("Please choose the directory to sync"), this);
        return false;
    }
    if (!QDir(path).exists()) {
        rayafileApp->warningBox(tr("The folder %1 does not exist").arg(path), this);
        return false;
    }

    if (mName->text().trimmed().isEmpty()) {
        rayafileApp->warningBox(tr("Please enter the name"), this);
        return false;
    }
    ...
    if (encrypted) {
        if (mPassword->text().trimmed().isEmpty() ||
            ...
            rayafileApp->warningBox(tr("Please enter the password"), this);
            return false;
        }
        ...
        if (passwd != passwdAgain) {
            rayafileApp->warningBox(tr("Passwords don't match"), this);
            return false;
        }
        passwd_ = passwd;
    } else {
    ...
    QString error;
    if (rayafileApp->rpcClient()->checkPathForClone(path, &error) < 0) {
        if (error.isEmpty()) {
            error = tr("Unknown error");
        }
        rayafileApp->warningBox(translateErrorMsg(error), this);
        return false;
    }
...
void CreateRepoDialog::createSuccess(const RepoDownloadInfo& info)
{
    ...
    int ret = rayafileApp->rpcClient()->cloneRepo(
        info.repo_id,
        info.repo_version,
        info.repo_name,
        path_,
