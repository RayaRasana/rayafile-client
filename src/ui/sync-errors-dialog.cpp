...
void SyncErrorsDialog::showEvent(QShowEvent *event)
{
    ...
    rayafileApp->trayIcon()->setSyncErrorStatus(false);
}
...
void SyncErrorsTableView::onDeleteFileAsyncError()
{
    foreach(const SyncError& error, selected_sync_errors_) {
        bool success = rayafileApp->rpcClient()->deleteFileAsyncErrorById(error.id);
        if (!success) {
            rayafileApp->messageBox(tr("Delete file sync error failed"));
            return;
        }
    }
    emit refreshModel();
...
void SyncErrorsTableView::onItemDoubleClicked(const QModelIndex& index)
{
    ...
    if (!error.repo_id.isEmpty()) {
        LocalRepo repo;
        rayafileApp->rpcClient()->getLocalRepo(error.repo_id, &repo);
        if (repo.isValid()) {
      ...
      error_column_width_(kErrorColumnWidth)
{
    ...
    connect(this, SIGNAL(sigSyncErrorUpdated()), rayafileApp->trayIcon(), SLOT(slotSyncErrorUpdate()));
...
void SyncErrorsTableModel::updateErrors()
{
    ...
    bool success = rayafileApp->rpcClient()->getSyncErrors(&errors, 0, 50);
    if (!success) {
