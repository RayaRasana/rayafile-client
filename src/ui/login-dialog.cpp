...
QStringList getUsedServerAddresses()
{
    ...
    settings.endGroup();
    QString preconfigure_addr = rayafileApp->readPreconfigureExpandedString(kPreconfigureServerAddr);
...
LoginDialog::LoginDialog(QWidget *parent) : QDialog(parent)
{
    ...
    QString preconfigure_addr = rayafileApp->readPreconfigureExpandedString(kPreconfigureServerAddr);
    if (rayafileApp->readPreconfigureEntry(kPreconfigureServerAddrOnly).toBool() && !preconfigure_addr.isEmpty()) {
    ...
    QString computerName = rayafileApp->settingsManager()->getComputerName();
...
void LoginDialog::doLogin()
{
    ...
    Account account =  rayafileApp->accountManager()->getAccountByHostAndUsername(url_.host(), username_);
...
void LoginDialog::onSslErrors(QNetworkReply* reply, const QList<QSslError>& errors)
{
    ...
    if (rayafileApp->detailedYesOrNoBox(tr("<b>Warning:</b> The ssl certificate of this server is not trusted, proceed anyway?"),
...
bool LoginDialog::validateInputs()
{
    ...
    rayafileApp->settingsManager()->setComputerName(computer_name_);
...
void LoginDialog::onFetchAccountInfoSuccess(const AccountInfo& info)
{
    ...
    rayafileApp->accountManager()->setCurrentAccount(account);
    rayafileApp->accountManager()->updateAccountInfo(account, info);
...
{
    QString server_addr =
        rayafileApp->readPreconfigureEntry(kPreconfigureShibbolethLoginUrl)
            .toString()
            .trimmed();
    if (!server_addr.isEmpty()) {
    ...
    if (server_addr.isEmpty()) {
        ...
        server_addr = rayafileApp->settingsManager()->getLastShibUrl();
        if (!getShibLoginUrl(server_addr, &url)) {
            return;
        }
    }

    rayafileApp->settingsManager()->setLastShibUrl(url.toString());
    sso_server_ = url;
