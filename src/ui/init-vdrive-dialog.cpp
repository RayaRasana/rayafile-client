...
{
    int ret;
    QString worktree = rayafileApp->configurator()->worktreeDir();
    QString error;

    ret = rayafileApp->rpcClient()->downloadRepo(info.repo_id,
...
void InitVirtualDriveDialog::checkDownloadProgress()
{
    ...
    if (rayafileApp->rpcClient()->getCloneTasks(&tasks) < 0) {
        return;
    }
    ...
    LocalRepo repo;
    rayafileApp->rpcClient()->getLocalRepo(default_repo_id_, &repo);
    default_repo_path_ = repo.worktree;
    finish();
}
