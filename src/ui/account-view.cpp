    ...
    : QWidget(parent)
{
    ...
    connect(rayafileApp->accountManager(), SIGNAL(requireAddAccount()),
...
void AccountView::showAddAccountDialog()
{
    ...
    if (dialog.exec() == QDialog::Accepted
        ...
        InitVirtualDriveDialog dialog(account_mgr->currentAccount(), rayafileApp->mainWindow());
...
void AccountView::deleteAccount()
{
    ...
    if (rayafileApp->yesOrNoBox(question, this, false)) {
        FileBrowserManager::getInstance()->closeAllDialogByAccount(account);
        QString error;
        QUrl server_url = account.serverUrl;
        server_url.setPath("/");
        if (rayafileApp->rpcClient()->unsyncReposByAccount(server_url,
                                                          account.username,
                                                          &error) < 0) {
        ...
        rayafileApp->accountManager()->removeAccount(account);
    }
}
...
{
    const std::vector<Account>& accounts = rayafileApp->accountManager()->accounts();
...
void AccountView::toggleAccount()
{
    ...
    // logout Account
    FileBrowserManager::getInstance()->closeAllDialogByAccount(account);
    rayafileApp->accountManager()->logoutDevice(account);
}
...
void AccountView::setupSortingMenu()
{
    ...
    auto order = rayafileApp->settingsManager()->repoSortOrder();
    ...
    connect(order_by_mtime, &QAction::triggered, [=]() {
        order_by_mtime->setIcon(QIcon(":/images/account-checked.png"));
        order_by_name->setIcon(QIcon(":/images/account-else.png"));
        rayafileApp->settingsManager()->setRepoSortOrder(RepoTreeModel::SORT_BY_LAST_UPDATED);
        emit sortOrderUpdated();
    });
    connect(order_by_name, &QAction::triggered, [=]() {
        order_by_mtime->setIcon(QIcon(":/images/account-else.png"));
        order_by_name->setIcon(QIcon(":/images/account-checked.png"));
        rayafileApp->settingsManager()->setRepoSortOrder(RepoTreeModel::SORT_BY_NAME);
        emit sortOrderUpdated();
    });
};
