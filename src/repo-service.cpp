...
void RepoService::start()
{
    ...
    if (!synced_subfolder_db_) {
        do {
            QString db_path = QDir(rayafileApp->configurator()->seafileDir()).filePath("accounts.db");
...
void RepoService::refreshLocalRepoList() {
    ...
    if (rayafileApp->rpcClient()->listLocalRepos(&local_repos_) < 0) {
    ...
    if (synced_subfolder_db_) {
        ...
        for (size_t i = 0; i < local_repos_.size(); ++i) {
            ...
            if (!found) {
                ...
                rayafileApp->rpcClient()->unsync(repo.id);
            }
        }
    }
...
void RepoService::onRefreshFailed(const ApiError& error)
{
    ...
    if (error.type() == ApiError::HTTP_ERROR && error.httpErrorCode() == 401) {
        rayafileApp->accountManager()->invalidateCurrentLogin();
        return;
    }
                                ...
                                QWidget *dialog_parent)
{
    ...
    rayafileApp->rpcClient()->getLocalRepo(repo_id, &r);
...
void RepoService::startGetRequestFor(const QString &repo_id)
{
    ...
    GetRepoRequest *req = new GetRepoRequest(rayafileApp->accountManager()->currentAccount(), repo_id);
...
void RepoService::wipeLocalFiles()
{
    ...
    rayafileApp->rpcClient()->listLocalRepos(&local_repos_);
    QStringList worktrees;
    for (size_t i = 0; i < local_repos_.size(); ++i) {
        const LocalRepo& repo = local_repos_[i];

        QString repo_server_url;
        if (rayafileApp->rpcClient()->getRepoProperty(repo.id, "server-url", &repo_server_url) < 0) {
            continue;
        }
