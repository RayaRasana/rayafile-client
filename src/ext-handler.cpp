                                                ...
                                                bool internal)
{
    ...
    const Account account = rayafileApp->accountManager()->getAccountByRepo(repo_id);
    if (!account.isValid()) {
        return;
    }
...
void SeafileExtensionHandler::onGetSharedLinkFailed(const ApiError& error) {
    ...
    rayafileApp->warningBox(tr("failed to get share link: %1").arg(error_msg));
}
...
void SeafileExtensionHandler::onLockFileSuccess()
{
    ...
    LocalRepo repo;
    rayafileApp->rpcClient()->getLocalRepo(req->repoId(), &repo);
    if (repo.isValid()) {
...
void SeafileExtensionHandler::onLockFileFailed(const ApiError& error)
{
    ...
    rayafileApp->warningBox(QString("%1: %2").arg(str, error.toString()));
}

void SeafileExtensionHandler::getUploadLink(const QString& repo_id, const QString& path_in_repo)
{
    const Account account =
            rayafileApp->accountManager()->getAccountByRepo(repo_id);
    if (!account.isValid()) {
        return;
    }
...
void SeafileExtensionHandler::onGetUploadLinkFailed(const ApiError& error)
{
    ...
    rayafileApp->messageBox(tr("Failed to get upload link: %1").arg(error_msg));
    req->deleteLater();
}
