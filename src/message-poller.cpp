...
void MessagePoller::start()
{
    ...
    connect(rayafileApp->daemonManager(), SIGNAL(daemonDead()), this, SLOT(onDaemonDead()));
...
void MessagePoller::processNotification(const SyncNotification& notification)
{
    ...
    if (type == "transfer") {
        ...
        rayafileApp->trayIcon()->showMessage(title, translateCommitDesc(desc), repo_id, commit_id, previous_commit_id);
        ...
        rayafileApp->trayIcon()->showMessage(title, msg, repo_id,
        ...
        rayafileApp->trayIcon()->showMessage(getBrand(), buf);
        ...
        if (rayafileApp->deletingConfirmationBox(text, info)) {
