...
void FileCache::cleanCurrentAccountCache()
{
    ...
    foreach(const QString& key, cache_->keys()) {
        const Account account = rayafileApp->accountManager()
...
void FileCache::start()
{
    ...
    QString db_path = QDir(rayafileApp->configurator()->seafileDir()).filePath("autoupdate-cache.db");
    if (sqlite3_open (toCStr(db_path), &db)) {
        ...
        rayafileApp->errorAndExit(QObject::tr("failed to open file cache database"));
        return;
    }
