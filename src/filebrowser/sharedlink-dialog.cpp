...
void SharedLinkDialog::onCreateSharedLinkFailed(const ApiError& error)
{
    ...
    if (error.type() == ApiError::HTTP_ERROR && error.httpErrorCode() == 400) {
        rayafileApp->warningBox(tr("Failed to generate share link: %1").arg(req->errorMsg()));
        return;
    }

    rayafileApp->warningBox(tr("Failed to generate share link: %1").arg(error.toString()));
}
