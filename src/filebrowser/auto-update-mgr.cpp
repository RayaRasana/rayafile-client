...
void AutoUpdateManager::cleanCachedFile()
{
    ...
    const Account cur_account = rayafileApp->accountManager()->currentAccount();
...
void AutoUpdateManager::uploadFile(const QString& local_path, bool replace_previous)
{
    ...
    FileNetworkTask *task = rayafileApp->dataManager()->createUploadTask(
...
void AutoUpdateManager::onUpdateTaskFinished(bool success)
{
    ...
    if (!success) {
        ...
        rayafileApp->trayIcon()->showMessage(tr("Upload Failure: %1").arg(error_msg),
                                            msg,
                                            task->repoId());
...
void AutoUpdateManager::onGetRepoSuccess(const ServerRepo& repo, QString repo_id, QString path, QString sig, QString file_id, QString local_path)
{
    ...
    rayafileApp->trayIcon()->showMessage(tr("Upload Success"),
...
{
    QString file_cache_dir = pathJoin(rayafileApp->configurator()->seafileDir(),
                               kFileCacheTopDirName);
    QString file_cache_tmp_dir = pathJoin(rayafileApp->configurator()->seafileDir(),
