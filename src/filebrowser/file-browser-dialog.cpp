      ...
      has_password_dialog_(false)
{
    ...
    data_mgr_ = rayafileApp->dataManager();
    ...
    AccountManager* account_mgr = rayafileApp->accountManager();
...
bool FileBrowserDialog::handleDragDropEvent(QObject *obj, QEvent *event)
{
    if (event->type() == QEvent::DragEnter) {
        ...
        if (current_readonly_) {
            rayafileApp->warningBox(tr("You do not have permission to upload to this folder"), this);
        } else {
...
{
    if (!rayafileApp->accountManager()->currentAccount().isValid()) {
...
{
    QString name = rayafileApp->getText(this, tr("Create a folder"),
        tr("Folder name"));
    name = name.trimmed();
    ...
    // invalid name
    if (name.contains("/")) {
        rayafileApp->warningBox(tr("Invalid folder name!"), this);
        return;
    }
...
void FileBrowserDialog::onDirentSaveAs(const QList<const SeafDirent*>& dirents)
{
    ...
    if (dirents.size() == 1) {
        ...
        if (QFileInfo(local_path).exists()) {
          ...
          if (!QFile::remove(local_path)) {
              rayafileApp->warningBox(tr("Unable to remove file \"%1\""), this);
              return;
          }
        }
    ...
    Q_FOREACH(const SeafDirent* dirent, dirents) {
        ...
        if  (QFileInfo(local_path).exists()) {
              if (!rayafileApp->yesOrNoBox(tr("Do you want to overwrite the existing file \"%1\"?").arg(dirent->name))) {
                  return;
              }
              if (!QFile::remove(local_path)) {
                  rayafileApp->warningBox(tr("Unable to remove file \"%1\""), this);
...
void FileBrowserDialog::onFileClicked(const SeafDirent& file)
{
    ...
    LocalRepo repo;
    if (rayafileApp->rpcClient()->getLocalRepo(repo_.id, &repo) == 0) {
        QString synced_path = ::pathJoin(repo.worktree, fpath);
        if (!QFileInfo(synced_path).exists()) {
            rayafileApp->warningBox(tr("File \"%1\" haven't been synced").arg(file.name));
            return;
        }
...
void FileBrowserDialog::uploadOrUpdateFile(const QString& path)
{
    ...
    if (findConflict(name, table_model_->dirents())) {
        QMessageBox::StandardButton ret = rayafileApp->yesNoCancelBox(
...
{
    if (new_name.isEmpty()) {
        new_name = rayafileApp->getText(this, tr("Rename"),
...
{
    if (rayafileApp->yesOrNoBox(dirent.isFile() ?
...
{
    if (!rayafileApp->yesOrNoBox(tr("Do you really want to delete these items"), this, false))
        return;
...
void FileBrowserDialog::onGetUploadLinkFailed(const ApiError&) {
    ...
    rayafileApp->messageBox(tr("Failed to get upload link information for file \"%1\"").arg(file));
    req->deleteLater();
}
...
{
    rayafileApp->warningBox(tr("Failed to get link"));
}
...
{
    rayafileApp->warningBox(tr("Create folder failed"), this);
}
...
{
    rayafileApp->warningBox(tr("Lock file failed"), this);
}
...
{
    rayafileApp->warningBox(tr("Rename failed"), this);
}
...
{
    rayafileApp->warningBox(tr("Remove failed"), this);
}
...
{
    rayafileApp->warningBox(tr("Share failed"), this);
}
...
void FileBrowserDialog::onGetDirentsPaste()
{
    if (repo_id_to_be_pasted_from_ == repo_.id) {
        if (current_path_ == dir_path_to_be_pasted_from_) {
            rayafileApp->warningBox(tr("Cannot paste files from the same folder"), this);
            return;
        }
        ...
        for (const QString& name : file_names_to_be_pasted_.keys()) {
            ...
            if (appendTrailingSlash(current_path_).startsWith(file_path_to_be_pasted)) {
                rayafileApp->warningBox(tr("Cannot paste the folder to its subfolder"), this);
                return;
            }
        }
    }
...
{
    rayafileApp->warningBox(tr("Copy failed"), this);
}
...
{
    rayafileApp->warningBox(tr("Move failed"), this);
}
...
void FileBrowserDialog::onOpenLocalCacheFolder()
{
     ...
     if (!::createDirIfNotExists(folder)) {
         rayafileApp->warningBox(tr("Unable to create cache folder"), this);
         return;
     }
     if (!openUrl(QUrl::fromLocalFile(folder)))
         rayafileApp->warningBox(tr("Unable to open cache folder"), this);
}
...
void FileBrowserDialog::onCreateSubrepoSuccess(const ServerRepo &repo)
{
    ...
    if (rayafileApp->rpcClient()->hasLocalRepo(repo.id)) {
        has_local = true;
    } else {
...
{
    rayafileApp->warningBox(tr("Create library failed!"), this);
}
