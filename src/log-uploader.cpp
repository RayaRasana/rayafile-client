...
void LogDirUploader::onCompressFinished(bool success, const QString& compressed_file_name)
{
    ...
    if (success) {
        ...
        rayafileApp->warningBox(tr("Upload log files failed"));
        onFinished();
    }
}
...
void LogDirUploader::onUploadLogDirFinished(bool success)
{
    ...
    if (!success) {
        ...
        rayafileApp->messageBox(tr("Successfully uploaded log files"));
    }
    onFinished();
    emit finished();
}
...
void LogDirCompressor::run() {
    ...
    QString username = rayafileApp->accountManager()->currentAccount().username;
