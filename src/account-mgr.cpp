...
int AccountManager::start()
{
    ...
    if (sqlite3_open (toCStr(db_path), &db)) {
        ...
        rayafileApp->errorAndExit(tr("failed to open account database"));
        return -1;
    }
...
void AccountManager::serverInfoSuccess(const ServerInfo &info)
{
    ...
    url.setPath("/");
    rayafileApp->rpcClient()->setServerProperty(
...
void AccountManager::clearSyncToken(const Account& account)
{
    ...
    QUrl url = account.serverUrl;
    url.setPath("/");
    if (rayafileApp->rpcClient()->removeSyncTokensByAccount(url,
                                                           account.username,
                                                           &error)  < 0) {
...
{
    return getAccountByRepo(repo_id, rayafileApp->rpcClient());
}
...
void AccountManager::invalidateCurrentLogin()
{
    ...
    clearSyncToken(account);
    clearAccountToken(account);
    rayafileApp->warningBox(tr("Authorization expired, please re-login"));
...
void AccountManager::onGetRepoTokensSuccess()
{
    ...
    foreach (const QString& repo_id, req->repoTokens().keys()) {
        rayafileApp->rpcClient()->setRepoToken(
