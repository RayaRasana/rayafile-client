...
void SettingsManager::loadSettings()
{
    ...
    if (rayafileApp->rpcClient()->seafileGetConfig("notify_sync", &str) >= 0)
        bubbleNotifycation_ = (str == "off") ? false : true;

    if (rayafileApp->rpcClient()->seafileGetConfigInt("download_limit",
                                                     &value) >= 0)
        maxDownloadRatio_ = value >> 10;

    if (rayafileApp->rpcClient()->seafileGetConfigInt("upload_limit",
                                                     &value) >= 0)
        maxUploadRatio_ = value >> 10;

    if (rayafileApp->rpcClient()->seafileGetConfig("allow_invalid_worktree",
                                                  &str) >= 0)
        allow_invalid_worktree_ = (str == "true") ? true : false;

    if (rayafileApp->rpcClient()->seafileGetConfig("sync_extra_temp_file",
                                                  &str) >= 0)
        sync_extra_temp_file_ = (str == "true") ? true : false;

    if (rayafileApp->rpcClient()->seafileGetConfig(
            "allow_repo_not_found_on_server", &str) >= 0)
        allow_repo_not_found_on_server_ = (str == "true") ? true : false;

    if (rayafileApp->rpcClient()->seafileGetConfig("disable_verify_certificate",
                                                  &str) >= 0)
        verify_http_sync_cert_disabled_ = (str == "true") ? true : false;

    if (rayafileApp->rpcClient()->seafileGetConfigInt("delete_confirm_threshold",
                                                     &value) >= 0)
...
void SettingsManager::loadProxySettings()
{
    ...
    QString use_proxy;
    rayafileApp->rpcClient()->seafileGetConfig(kUseProxy, &use_proxy);
    if (use_proxy.isNull()) {
    ...
    rayafileApp->rpcClient()->seafileGetConfig(kUseSystemProxy, &use_system_proxy);
    ...
    if (rayafileApp->rpcClient()->seafileGetConfig(kProxyAddr, &proxy_host) <
        0) {
        return;
    }
    if (rayafileApp->rpcClient()->seafileGetConfigInt(kProxyPort, &proxy_port) <
        0) {
        return;
    }
    if (rayafileApp->rpcClient()->seafileGetConfig(kProxyType, &proxy_type) <
        0) {
        return;
    }
    if (proxy_type == "http") {
        if (rayafileApp->rpcClient()->seafileGetConfig(kProxyUsername,
                                                      &proxy_username) < 0) {
            return;
        }
        if (rayafileApp->rpcClient()->seafileGetConfig(kProxyPassword,
                                                      &proxy_password) < 0) {
            return;
        }
...
{
    if (rayafileApp->rpcClient()->setAutoSync(auto_sync) < 0) {
        // Error
        return;
    }
    auto_sync_ = auto_sync;
    rayafileApp->trayIcon()->setState(
...
void SettingsManager::setNotify(bool notify)
{
    if (bubbleNotifycation_ != notify) {
        if (rayafileApp->rpcClient()->seafileSetConfig(
...
void SettingsManager::setMaxDownloadRatio(unsigned int ratio)
{
    if (maxDownloadRatio_ != ratio) {
        if (rayafileApp->rpcClient()->setDownloadRateLimit(ratio << 10) < 0) {
            // Error
            return;
        }
...
void SettingsManager::setMaxUploadRatio(unsigned int ratio)
{
    if (maxUploadRatio_ != ratio) {
        if (rayafileApp->rpcClient()->setUploadRateLimit(ratio << 10) < 0) {
            // Error
            return;
        }
...
void SettingsManager::setHideDockIcon(bool hide)
{
    ...
    utils::mac::orderFrontRegardless(rayafileApp->mainWindow()->winId());
#endif
}
...
void SettingsManager::setDeleteConfirmThreshold(int value)
{
    if (delete_confirm_threshold_ != value) {
        if (rayafileApp->rpcClient()->seafileSetConfigInt(
...
void SettingsManager::setComputerName(const QString &computerName)
{
    ...
    rayafileApp->rpcClient()->seafileSetConfig("client_name", computerName);
}
...
{
    QString str;
    rayafileApp->rpcClient()->seafileGetConfig(kHideWindowsIncompatiblePathNotification, &str);
    return str != "false";
}
...
void SettingsManager::setHideWindowsIncompatibilityPathMsg(bool enabled)
{
    ...
    rayafileApp->rpcClient()->seafileSetConfig(kHideWindowsIncompatiblePathNotification, set_value);
    return;
}
...
{
    QString str;
    rayafileApp->rpcClient()->seafileGetConfig(kIgnoreSymlinks, &str);
    return str == "true";
}
...
{
    QString set_value = enabled == true ? "true" : "false";
    rayafileApp->rpcClient()->seafileSetConfig(kIgnoreSymlinks, set_value);
    return;
}
#endif
...
void SettingsManager::onSystemProxyPolled(const QNetworkProxy &system_proxy)
{
    ...
    if (proxy.type == NoProxy) {
        ...
        rayafileApp->rpcClient()->seafileSetConfig(kProxyType, "none");
    } else {
        writeProxyDetailsToDaemon(proxy);
    }
}
